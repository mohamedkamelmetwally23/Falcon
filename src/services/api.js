...
export const orderApi = {
  ...
  status: (id, status) => api(`/orders/${id}/status`, { method: 'PATCH', body: JSON.stringify({ status }) }),
  confirm: id => api(`/orders/${id}/confirm`, { method: 'POST' }),
  reject: id => api(`/orders/${id}/reject`, { method: 'POST' }),
};
export const returnApi = {
  list: () => api('/returns'),
  create: data => api('/returns', { method: 'POST', body: JSON.stringify(data) }),
};
